...
export function isNoneOf(paramName = UNKNOWN, param: any, contains: Array<any> = []) {
  if (!contains.includes(param)) {
    throwError(getCaller(2), paramName, `to be any of ${JSON.stringify(contains)}`);
  }
}

export function isAnyOf(paramName = UNKNOWN, param: any, contains: Array<any> = []) {
  if (contains.includes(param)) {
    throwError(getCaller(2), paramName, `not to be any of ${JSON.stringify(contains)}`);
  }
}

export function isNotType(paramName = UNKNOWN, param: any, type = '') {
  if (getType(param) !== type.toLowerCase()) {
    throwError(getCaller(2), paramName, `to be type ${type.toLowerCase()}`);
  }
}

export function isAnyTypeOf(paramName = UNKNOWN, param: any, types: Array<string> = []) {
  types.forEach(type => {
...
export function missingKey(paramName = UNKNOWN, param: any, keyName = '') {
...
export function missingAnyKeys(paramName = UNKNOWN, param: any, keyNames: Array<string> = []) {
  isUndefined(paramName, param);
...
export function containsUndefined(paramName = UNKNOWN, param: any) {
...
export function isInvalidPath(paramName = UNKNOWN, param: any) {
  isUndefined(paramName, param);
...
export function isInvalidWriteData(paramName = UNKNOWN, param: any) {
  isUndefined(paramName, param);
